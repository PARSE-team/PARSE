...
class PDS3Label(OrderedDict):
    ...
    def _format_dict(self, d, indent=0):
        ...
        lister = [('Paul', 23), ('Syd', 25), ('Scott', 18)]
        dict = OrderedDict(lister)
        countah = 0

        def format_key(k):
            return ('{:' + str(n) + '}').format(k)
        ...
        for k, v in d.items():
            ...
            if isinstance(v, (PDS3Object, PDS3Group)):
                ...
                # countah += 1
                # print("\n\nPDS3Objects encountered: {}".format(countah) + "\n")
                ...
                # FIXME: it thinks that the TABLE object has been converted to a list, and it can just iterate through
                """
                iterate through TABLE (PDSObject = OrderedDict)
                    cast each 
                """
                print('\n\n')
